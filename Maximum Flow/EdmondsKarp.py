...
def edmonds_karp(graph, source, sink):
    ...
    max_flow = nx.maximum_flow_value(flow_network, source, sink, flow_func=nx.algorithms.flow.edmonds_karp)
    return max_flow
...
# Calculate max flows using Edmonds-Karp algorithm
predicted_max_flows = [edmonds_karp(graph, source, sink) for (graph, (source, sink)) in zip(graphs, sources_sinks)]
